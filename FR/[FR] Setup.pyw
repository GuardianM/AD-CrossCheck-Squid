...
# ---> Fonction executée si l'utilisateur ferme la fenetre
    ...
    # Un message indiquant les rubriques non renseignées est affiché
        if self.sshState == 0 and self.dbState == 0 and self.shareState == 0:
            if messagebox.askokcancel("Quitter", "Aucune des rubriques n'a été paramétrées. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()

        elif self.sshState == 1 and self.dbState == 0 and self.shareState == 0:
            if messagebox.askokcancel("Quitter", "Les rubriques Base de données et Dossier partagé n'ont pas été paramétrées. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()

        elif self.sshState == 0 and self.dbState == 1 and self.shareState == 0:
            if messagebox.askokcancel("Quitter", "Les rubriques PfSense et Dossier partagé n'ont pas été paramétrées. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()


        elif self.sshState == 0 and self.dbState == 0 and self.shareState == 1:
            if messagebox.askokcancel("Quitter", "Les rubriques Base de données et PfSense n'ont pas été paramétrées. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()

        elif self.sshState == 1 and self.dbState == 1 and self.shareState == 0:
            if messagebox.askokcancel("Quitter", "La rubrique Dossier partagé n'a pas été paramétrée. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()

        elif self.sshState == 1 and self.dbState == 0  and self.shareState == 1:
            if messagebox.askokcancel("Quitter", "La rubrique Base de données n'a pas été paramétrée. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()

        elif self.sshState == 0 and self.dbState == 1  and self.shareState == 1:
            if messagebox.askokcancel("Quitter", "La rubrique Pfsense n'a pas été paramétrée. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()

        elif self.sshState == 1 and self.dbState == 1  and self.shareState == 1:
            if messagebox.askokcancel("Quitter", "Toutes les données ont été enregistrées. Souhaitez-vous vraiment quitter cette application ?", parent=self):
                app.destroy()
...
# ---> Structure de la fenetre main        
    ...
    # LabelFrame pour les parties bdd, pfsense et dossier partagé
        self.db_f = ttk.LabelFrame(self.cred_frame, text="   Base de données   ", labelanchor="n")
        ...
        self.computer_f = ttk.LabelFrame(self.cred_frame, text="   Dossier partagé   ", labelanchor="n")
    ...
    # Message de "bienvenue"  
        self.welcomeLabel = tk.Label(self.top_frame, bg="#343434", fg="#D2A028",
                                     text="Paramétrage du moteur de recherche d'activité des utilisateurs !", wrap=305, font=('Century Gothic',13))
...
# ---> Partie base de données
    ...
    # Champ Username
        self.db_UsernameLabel = tk.Label(self.db_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Nom d'utilisateur")
    ...
    # Champ password
        self.db_PasswordLabel = tk.Label(self.db_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Mot de passe")
    ...
    # Champ Ip address
        self.db_IpLabel = tk.Label(self.db_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Adresse IP")
    ...
    # Champ DB name
        self.db_NameLabel = tk.Label(self.db_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Nom de la BDD")
    ...
    # Bouton valider       
        self.db_button = ttk.Button(self.db_f, text="Valider", command=self.DbVerification)
...
# ---> Partie pfsense
    ...
    # Message d'entete
        self.pf_message = tk.Label(self.pfsense_f, bg="#343434", fg="white", font=('Century Gothic',11),
                                   text="Authentification via SSH")
        self.pf_message.grid(row=0, columnspan=2, sticky="n", pady=(15, 15))                     

    # Champ SSH Username
        self.pf_UsernameLabel = tk.Label(self.pfsense_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Nom d'utilisateur")
    ...
    # Champ SSH password
        self.pf_PasswordLabel = tk.Label(self.pfsense_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Mot de passe")
    ...
    # Champ Ip address
        self.pf_IpLabel = tk.Label(self.pfsense_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Adresse IP")
    ...
    # Champ Root password
        ...
        self.pf_RootLabel = tk.Label(self.pfsense_f, bg="#343434", fg="white", font=('Century Gothic',11), text="Mot de passe ROOT")
    ...
    # Bouton valider
        self.pf_button = ttk.Button(self.pfsense_f, text="Valider", command=self.SshVerification)
...
# ---> Partie computer
    def ComputerStructure(self):
        ...
        self.computer_message = tk.Label(self.computer_f, bg="#343434", fg="white", font=('Century Gothic',11),
                                   text="Entrez le nom du controleur de domaine ci-dessous ")
        self.computer_message.grid(row=0, columnspan=4, sticky="n", pady=(15,10), padx=(10,10))                     


    # Message d'avertissement
        self.computer_avert_message = tk.Label(self.computer_f, bg="#343434", fg="red", font=('Century Gothic',10,'italic'), text="Faites attention aux majuscules et minuscules !")
    ...
    # Bouton valider
        self.computer_button = ttk.Button(self.computer_f, text="Valider", command=self.CheckShare)
...
# ---> Verification de l'existence du dossier ShareUsersLogs sur le controleur de domaine
    ...
    # Test de l'accessibilité au repertoire partagé et aux scripts. Message d'erreur en fonction de l'accessiblité de chacun
        if os.path.isdir(self.path1) and os.path.isfile(self.path2) and os.path.isfile(self.path3):
            messagebox.showinfo("Bravo !", "Accès au répertoire partagé SharedUsersLogs et aux scripts login/login.bat réussi !", parent=self)
            self.shareState += 1
            self.computer_button.config(text="Réussi !", command=self.AlreadyDone)
            ...
            messagebox.showerror("Erreur", "Accès au script logout.bat impossible.", parent=self)
            

        elif os.path.isdir(self.path1) and not os.path.isfile(self.path2) and os.path.isfile(self.path3):
            messagebox.showerror("Erreur", "Accès au script login.bat impossible.", parent=self)


        elif not os.path.isdir(self.path1) and os.path.isfile(self.path2) and os.path.isfile(self.path3):
            messagebox.showerror("Erreur", "Accès au répertoire partagé SharedUsersLogs impossible.", parent=self)


        elif not os.path.isdir(self.path1) and not os.path.isfile(self.path2) and os.path.isfile(self.path3):
            messagebox.showerror("Erreur", "Accès au répertoire partagé SharedUsersLogs et au script login.bat impossible.", parent=self)


        elif not os.path.isdir(self.path1) and os.path.isfile(self.path2) and not os.path.isfile(self.path3):
            messagebox.showerror("Erreur", "Accès au répertoire partagé SharedUsersLogs et au script logout.bat impossible.", parent=self)


        elif not os.path.isdir(self.path1) and not os.path.isfile(self.path2) and not os.path.isfile(self.path3):
            messagebox.showerror("Erreur", "Accès au répertoire partagé SharedUsersLogs et aux scripts logoin/logout.bat impossible.", parent=self)

        else:
            messagebox.showerror("Erreur", "Un problème a été rencontré.", parent=self)               
            return
...
# ---> Fonction de verification de la connectivité à la base de données
    ...
    # Test de connexion
        try:
            ...
            messagebox.showinfo("Information", "Connexion à la base de données réussie !", parent=self)
            self.CreateTables()
...
# --->  Fonction de creation des tables dans la BDD
    def CreateTables(self):
        messagebox.showinfo("Information", "Création des tables en cours, patientez", parent=self)
        ...
        try:
            ...
            messagebox.showerror("Erreur", "Impossible de créer les tables.", parent=self)               
            return

        else:
            messagebox.showinfo("Bravo !", "Création des tables réussie !", parent=self)
            self.dbState += 1
            self.db_button.config(text="Réussi !", command=self.AlreadyDone)
...
# ---> Fonction de verification de la connexion à PfSense via SSH
    def SshVerification(self):
        ...
        try:
            ...
            messagebox.showerror("Erreur", "Impossible de se connecter à PfSense via SSH.", parent=self)               
            return


        else:
            messagebox.showinfo("Information", "Connexion à PfSense réussie !", parent=self)
            self.pf_button.config(text="Réussi !", command=self.AlreadyDone)
...
# ---> Fonction liée au boutons "réussi" pour eviter de reecrire les informations (ssh, bdd et dossier partagé)
    def AlreadyDone(self):
        messagebox.showinfo("Information", "L'ensemble des paramètres liés à cette partie ont déjà été renseignés", parent=self)
...
# ---> Check final - On verifie si les script qui doivent etre utilisés par les taches plannifiés sont bien accessible
    def FinalCheck(self):
        messagebox.showinfo("Bravo !", "Toutes les informations sont correctes ! Encore une dernière vérification est vous en aurez fini avec le paramétrage de cette application :) ", parent=self)
    ...
    # Test de l'accessibilité des scripts. Message d'erreur en fonction de l'accessiblité de chacun
        if os.path.isfile("Taches_auto\csv_to_sql.py") and os.path.isfile("Taches_auto\proxy.py"):
            ...
            # Inscription du nom du PC dans le fichier sql_credentials qui sera utilisé par csv_to_sql
            with open("Taches_auto\sql_credentials.py", 'a') as f:
                f.write('DC_name = ' + '"' + self.computername + '"\n')
                f.close()


            
            messagebox.showinfo("Bravo !", "Vous pouvez fermer cette fenêtre (setup.pyw) et vous pourrez utiliser le moteur de recherche une fois que des données auront été enregistrées par les différentes taches planifiées.", parent=self)
    ...
    # Si le script csv_to_sql.py est introuvable mais que proxy.py existe
        ...
        # Message d'erreur
            messagebox.showerror("Erreur", "Impossible de trouver le script proxy.py dans le dossier Taches_auto.", parent=self)
        ...
        # Creation d'un bouton pour réessayer la validation
            self.retry_button = ttk.Button(self.top_frame, text="Réessayer la validation finale", style="Rb.TButton", command=self.FinalCheck)
    ...
    # Si le script csv_to_sql.py est introuvable mais que proxy.py existe
        ...
        # Message d'erreur
            messagebox.showerror("Erreur", "Impossible de trouver le script csv_to_sql.py dans le dossier Taches_auto.", parent=self)
    ...
    # Si csv_to_sql.py et proxy.py sont introuvables
        ...
        # Message d'erreur
            messagebox.showerror("Erreur", "Impossible de trouver les scripts csv_to_sql.py et proxy.py dans le dossier Taches_auto.", parent=self)
