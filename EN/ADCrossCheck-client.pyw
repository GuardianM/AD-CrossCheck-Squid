...
# ---> Fonction executée si l'utilisateur ferme la fenetre
    def ExitMain(self):
        if  messagebox.askokcancel("Quit", "Do you really wish to quit ?", parent=self) and second_count == 0:
            app.destroy()

        elif messagebox.askokcancel("Quit", "Do you really wish to quit ?", parent=self) and second_count >= 1:
...
# ---> Structure de la fenetre main        
    ...
    # Message de "bienvenue"
        self.welcomeLabel = tk.Label(self.top_frame, bg="#343434", fg="#D2A028",
                                     text="Welcome to the users activity search engine !", wrap=305, font=('Century Gothic',13))
        self.welcomeLabel.grid(row=1, pady=(15, 15))

        self.welcomeLabel2 = tk.Label(self.top_frame, bg="#343434", fg="white",
                                      text="Please choose a date below then press the OK button", font=('Century Gothic',11))
...
# ---> Fonction permettant une nouvelle recherche
    def ValidAction2(self):
        ...
        if username is None:
            messagebox.showinfo("Information", "Unable to retrieve database credentials, please enter them again", parent=self)
            DbW()
...
# ---> Fonction de verification de la date   
    ...
    # Sinon on indique à l'utilisateur que la date n'est pas utilisable
            ...
            messagebox.showerror("Error", "Date is unusable, please enter a date with the following format dd/mm/yyyy", parent=self)
...
# ---> Structure de la fenetre DbW
    def DbWStructure(self):
        ...
        self.db_message = tk.Label(self.top_frame, bg="#343434", fg="#D2A028", font=('Century Gothic',12),
                                   text="Please enter database IP and credentials below", wrap=300)
        self.db_message.grid(row=1, sticky="w", pady=(15, 15))
                          

    
    # Username
        self.db_UsernameLabel = tk.Label(self.DB_frame, bg="#343434", fg="white", font=('Century Gothic',11), text="Username")
    ...
    # Password
        self.db_PasswordLabel = tk.Label(self.DB_frame, bg="#343434", fg="white", font=('Century Gothic',11), text="Password")
    ...
    # Ip address
        self.db_IpLabel = tk.Label(self.DB_frame, bg="#343434", fg="white", font=('Century Gothic',11), text="IP address")
    ...
    # DB name
        self.db_NameLabel = tk.Label(self.DB_frame, bg="#343434", fg="white", font=('Century Gothic',11), text="Database name")
    ...
    # Bouton valider       
        self.db_button = ttk.Button(self.DB_frame, text="Enter", command=self.ValidAction)
...
# ---> Verification de la connexion à la BDD avec les informations d'authentifications fournies
    ...
    # Test de connexion
        try:
            ...
            messagebox.showerror("Erreur", "Can't connect to database", parent=self)               
            return
...
# --->  Action lorsque l'on essaye de quitter la fenetre
    def ExitSecondW(self):
        if  messagebox.askokcancel("Quit", "Do you really wish to quit ?", parent=self):
            self.destroy()
            app.destroy()
...
# ---> Menu de l'application
    ...
    # Premiere partie du menu nommée : File
        ...
        menuFile.add_command(label="New search", command=self.NewSearch)
        menuFile.add_command(label="Quit", command=self.ExitSecondW)


    # Deuxieme partie du menu nommée : Options
        menuOptions = tk.Menu(menuBar, tearoff=0)
        menuOptions.add_command(label="Show code")


    # Troisieme partie du menu nommée : Help
        menuHelp = tk.Menu(menuBar, tearoff=0)
        menuHelp.add_command(label="About this program", command=self.GoToGit)
        menuHelp.add_command(label="Go to GitHub", command=self.GoToGit)
...
# ---> Elements constituants la partie resultats       
    ...
    # Les messages d'informations de la partie resultats
        self.infoLabel1 = tk.Label(top_frame2, bg="#343434", fg="#D2A028", font=('Century Gothic',12), text="Below you will find the names of the users connected on " + date)
        self.infoLabel1.grid(row=1, column=0, pady=(15,10))

        self.infoLabel2 = tk.Label(top_frame2, bg="#343434", fg="white", font=('Century Gothic',11), text="Click on one of the buttons to display the selected user’s browsing history")
    ...
    # La partie information (tout en bas de la fenetre)
        self.infoLabe3 = tk.Label(information_frame2, bg="#343434", fg="white", font=('Century Gothic',9), text="Information : if a username appears several times, it indicates that it has logged in several times on the selected date")
...
# ---> Capture du nom d'utilisateur lié au bouton sur lequel on a cliqué 
    ...
    # Quand on aura cliqué sur l'un des boutons, de nouveaux boutons personnalisés apparaissent (exportation et nouvelle recherche)
        if self.bttn_clicks == 1:
            self.exportButton1 = ttk.Button(export_frame2, text="Export browsing history of " + selectedUser, command=self.Export)
            self.exportButton1.grid(row=1, column=0, pady=(10,20), padx=10)

            self.newSearchButton1 = ttk.Button(export_frame2, text="New search", command=self.NewSearch)
...
# ---> Fonction d'exportation 
    def Export(self):
        ...
        f.close()
        messagebox.showinfo("Information", "File Saved", parent=self)
...
# ---> Fonction de verification : Single search = 1 seule connexion de l'utilisateur dans la journée / Multi search = plusieurs connexion de l'utilisateur dans la même journée
    ...
    # On verifie pour l'utilisateur selectionné, les actions de Logion
    ...
    # Si plusieurs résultats sont trouvés on passe sur la fonction MultipleSearch
            ...
            print("Multiple session finded...")
            print("Searching for all information...")
            MultiSearch()
    ...
    def SearchLogoutHeure(self):
        ...
        if result.rowcount == 1:
            for row in result:
                global logout_heure
                logout_heure = row[0]
                print("Logout time :", logout_heure)
                self.SearchIp()
    ...
    def SearchIp(self):    
        ...
        if result.rowcount == 1:
            for row in result:
                global ip_selected
                ip_selected = row[0]
                print("Client IP address :", ip_selected)
                print("")
                print("*********************************************************")
                print("")
...
# ---> Fonction de recherche de l'historique de l'utilisateur
    ...
    # Si aucune réponse à la requete, cela veut dire que user c'est bien connecté mais n'a pas été sur internet
            ...
            print("No activity found on", date, "with IP address", ip_selected, "for the user", user)
    ...
    # Boucle permettant de recuperer les informations retournées precedement (heure de login, heure de lougout et ip)
        ...
        # Sinon, on capture le premier résultat de chaque requete SQL. On retire les possibles caracteres speciaux pour pouvoir exploiter les données
            ...
            # Impression de l'heure, de l'ip du poste ainsi que des URL visitées
                print("Here is the browsing history of", user, "for this session", session_count, ":")
                print("")
                for row in result:
                    print(row[1], row[2], row[3])
